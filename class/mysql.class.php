...
class mysql extends mysqli {
    ...
    function table(MySQLi_Result $result) {
        ...
        return sprintf('<div style="font-family:sans-serif; padding:10px; margin-top:5px; color:Black; ">Query:<br /><pre>%s</pre>Result:<br /><br /><table border="1"><tr>%s</tr>%s</table><br />%d Records</div>', htmlspecialchars(utf8_encode($this->sql_query)), $head, $body, $i);
    }
    ...
    function error($error_type) {
        ...
        switch ($error_type) {
            case 'OPEN':
                $text = 'MySQL Error: Could not connect or open database';
                break;

            case 'QUERY FAILED':
                $text = sprintf('MySQL Error: Following Query contained errors:<br /><pre>%s</pre><b>Error %d:</b><br />%s<br />', htmlspecialchars(utf8_encode($this->sql_query), ENT_QUOTES, 'UTF-8'), $this->errno, htmlspecialchars(utf8_encode($this->error), ENT_QUOTES, 'UTF-8'));
                break;

            case 'WRONG RESULT TYPE':
                $text = 'MySQL Error: Wrong result type';
                break;

            case 'NO.CONN':
                $text = 'MySQL Error: No database connection';
                break;
